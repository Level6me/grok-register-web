...
def get_configured_proxy():
    ...
    proxies_txt = '/home/ubuntu/grok-register/proxies.txt'
    if not proxy_list and os.path.exists(proxies_txt):
        try:
    ...
    return normalize_proxy_url(str(_config.get('proxy', '') or '').strip())
...
def _is_local_url(url):
    s = str(url or '').lower()
    return s.startswith('http://127.') or s.startswith('http://localhost') or s.startswith('http://0.0.0.0')
